...
memory=f"approximately {S['peak_MiB']:.1f} MiB peak resident memory" if S['peak_MiB'] is not None else 'peak resident memory unavailable on this platform'
...
'RUNTIME':f"The final numerical run used Python {S['python']} on {S['platform']} ({S['cpu']}). It took {S['seconds']:.2f} seconds, with {memory}. This is a single-process CPU computation, excluding downloads, spreadsheet extraction, figures and document rendering. No specialised hardware or GPU is required; wall time varies by machine.",
...
d.core_properties.created=d.core_properties.modified=datetime(2026,9,14,tzinfo=timezone.utc)
...
while i<len(lines):
 ...
 if line.startswith('|'):
  ...
  while i<len(lines) and lines[i].startswith('|'):
   ...
   i+=1
  chunks=[rows]  # Let Word paginate one continuous table with a repeated header.
